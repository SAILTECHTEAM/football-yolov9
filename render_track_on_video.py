...
def _draw_skeleton(img, kpts, color=(0, 220, 220), radius=3, thickness=2):
    ...
    if K >= 17:
        ...
        edges = [
            ...
            (5, 6), (11, 12)     # shoulders bar, hips bar
        ]
        for a, b in edges:
            if a < K and b < K:
...
def render_tracks_on_video(
    ...
    if people_tracks_jsonl:
        ppl_tracks, best_tid = load_people_tracks_with_scores(people_tracks_jsonl, score_field=score_field)
    ...
    while True:
        ...
        for item in ppl_tracks.get(frame_idx, []):
            ...
            if show_scores:
                ...
                ascr = item.get("assoc_score")
                label = f"person #{tid}"
                if dc == dc:   label += f" | conf {dc:.2f}"
                if ascr == ascr: label += f" | iou* {ascr:.2f}"
                if ts is not None:
                    try:
        ...
        if dets:
            existing = np.array(
                [it["bbox"] for it in ball_tracks.get(frame_idx, []) + ppl_tracks.get(frame_idx, [])],
                dtype=np.float32
            ) if (ball_tracks.get(frame_idx) or ppl_tracks.get(frame_idx)) else np.zeros((0,4), np.float32)
    ...
    if best_tid is None:
        ...
        print(f"[OK] wrote {out_path} (best person track with skeleton: tid={best_tid}, score_field={score_field})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--people-tracks", type=str, required=False, default=None, help="[single-file] people tracks JSONL path")
    parser.add_argument("--ball-tracks", type=str, required=False, default=None, help="[single-file] ball tracks JSONL path")
    parser.add_argument("--dets", type=str, required=False, default=None, help="[single-file] original detections JSONL path")
    parser.add_argument("--output", type=str, required=False, help="[single-file] output video path")

    # batch options
    parser.add_argument("--people-token", type=str, default=".cls_0.tracks.jsonl", help="[batch] substring to identify people tracks JSONL")
    parser.add_argument("--ball-token",   type=str, default=".cls_32.tracks.jsonl", help="[batch] substring to identify ball tracks JSONL")
    parser.add_argument("--dets-token",   type=str, default=".dets.jsonl", help="[batch] substring to identify dets JSONL; set empty to disable")
    parser.add_argument("--output-dir",   type=str, default=None, help="[batch] write outputs under this directory; mirrors subfolders from root")

    parser.add_argument("--people-color", type=str, default="0,220,0", help="BGR color for people boxes (default green)")
    parser.add_argument("--ball-color",   type=str, default="40,40,230", help="BGR color for ball boxes (default red-ish)")
    parser.add_argument("--det-color",    type=str, default="200,200,200", help="BGR color for unmatched dets (default light gray)")
    parser.add_argument("--thickness",    type=int, default=2, help="box thickness")
    parser.add_argument("--no-scores",    action="store_true", help="do not show det_conf/assoc_score on labels")
    parser.add_argument("--score-field",  type=str, default="team_score", help="per-track array score field to pick best person (e.g., team_score_b)")
