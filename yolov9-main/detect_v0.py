    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if use_slicer:
            ...
            for i, im0 in enumerate(im0s if isinstance(im0s, list) else [im0s]):
                ...
                current_slicer = sv.InferenceSlicer(callback=slicer_callback, slice_wh=slice_size)
                
                # Inference - dt[1]
                with dt[1]:
                    detections = current_slicer(im0)
                ...
                if len(detections):
                    ...
                    for class_id, confidence in zip(detections.class_id, detections.confidence):
                        n = (detections.class_id == class_id).sum()  # detections per class
                        s += f"{n} {names[int(class_id)]}{'s' * (n > 1)}, "  # add to string
